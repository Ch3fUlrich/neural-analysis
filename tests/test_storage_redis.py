...
class TestRedisCache:
    ...
    def test_redis_cache_initialization_both_branches(self) -> None:
        ...
        with patch("neural_analysis.utils.storage.redis_cache.REDIS_AVAILABLE", False):
            with patch(
                "neural_analysis.utils.storage.redis_cache.logger"
            ) as mock_logger:
                config = StorageConfig(use_redis=True)
                cache = RedisCache(config)
                assert not cache.is_available()
        ...
        with patch("neural_analysis.utils.storage.redis_cache.REDIS_AVAILABLE", True):
            with patch(
                "neural_analysis.utils.storage.redis_cache.logger"
            ) as mock_logger:
                config = StorageConfig(use_redis=False)
