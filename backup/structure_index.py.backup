"""
Structure Index Computation Module

This module computes the structure index (SI) metric, which quantifies how well 
the structure of behavioral variables (e.g., position, velocity) is preserved 
in neural representations.

The structure index works by:
1. Binning behavioral data (labels) into discrete groups
2. Computing pairwise overlap between neural activity in different behavioral bins
3. Calculating a normalized index where higher values indicate better structural preservation

The module also provides visualization tools to display overlap relationships as directed graphs.

Key Functions:
    compute_structure_index: Main function to compute the structure index
    draw_graph: Visualize bin-group overlap relationships as a directed graph
    cloud_overlap_neighbors: Compute overlap using k-nearest neighbors
    cloud_overlap_radius: Compute overlap using radius-based neighborhoods

Dependencies:
    - numpy: Numerical operations
    - matplotlib: Visualization
    - sklearn: Distance metrics and nearest neighbors
    - scipy: Spatial distance calculations
    - networkx: Graph visualization
    - faiss (optional): Fast similarity search for large datasets
    - tqdm: Progress bars
...
Example:
    ...
    >>> neural_data = np.random.rand(1000, 10)  # [n_samples, n_dimensions]
    >>> position = np.random.rand(1000, 2)      # [n_samples, n_features]
    >>> 
...
# Supported distance metrics for computing overlaps
...
def create_ndim_grid(label, n_bins, min_label, max_label, discrete_label):
    ...
    for nd in range(ndims):
        if discrete_label[nd]:
            # For discrete labels, create one bin per unique value
            grid_edges.append(np.tile(np.unique(label[:, nd]).reshape(-1, 1), (1, 2)))
        else:
            # For continuous labels, create evenly spaced bins
    ...
    # Generate the grid containing indices of points in each bin
    # and coordinates as the midpoint of edges
...
def cloud_overlap_radius(cloud1, cloud2, r, distance_metric):
    ...
    """
    # Stack both clouds into a single array for distance computation
    cloud_all = np.vstack((cloud1, cloud2)).astype("float32")
    idx_sep = cloud1.shape[0]  # Index separating the two clouds
    
    # Create labels identifying which cloud each point belongs to
    cloud_label = np.hstack((np.ones(cloud1.shape[0]), np.ones(cloud2.shape[0]) * 2))

    # Compute distance matrix based on chosen metric
    if distance_metric == "euclidean":
        D = distance_matrix(cloud_all, cloud_all, p=2)

    elif distance_metric == "geodesic":
        # Use Isomap to compute geodesic distances along manifold
        model_iso = Isomap(n_components=1)
        emb = model_iso.fit_transform(cloud_all)
        D = model_iso.dist_matrix_

    # Sort distances to find neighbors within radius
    I = np.argsort(D, axis=1)
    for row in range(I.shape[0]):
        D[row, :] = D[row, I[row, :]]
    I = I[:, 1:].astype("float32")  # Exclude self (distance 0)
    D = D[:, 1:]
    
    # Mark neighbors outside radius as NaN
    I[D > r] = np.nan
...
def cloud_overlap_neighbors(cloud1, cloud2, k, distance_metric):
    ...
    """
    # Stack both clouds into a single array
    cloud_all = np.vstack((cloud1, cloud2)).astype("float32")
    idx_sep = cloud1.shape[0]  # Index separating the two clouds
    ...
    # Handle edge case: if k >= total points, all points are neighbors
    if k > n_points - 1:
        overlap_1_2 = (
    ...
    if distance_metric == "euclidean":
        if use_fast:
            # Use FAISS for fast approximate nearest neighbors (if available)
            index = faiss.IndexFlatL2(cloud_all.shape[1])
            # Note: GPU acceleration could be enabled here if needed
            # if faiss.get_num_gpus() > 0:
            #     res = faiss.StandardGpuResources()
            #     index = faiss.index_cpu_to_gpu(res, 0, index)
            
            index.add(cloud_all)  # Add vectors to the index
            _, I = index.search(cloud_all, k + 1)  # +1 because first neighbor is self
            I = I[:, 1:]  # Remove self from neighbors
        else:
            # Fall back to sklearn if FAISS not available
        ...
        # Compute geodesic distances along manifold using Isomap
        model_iso = Isomap(n_components=1)
        emb = model_iso.fit_transform(cloud_all)
...
def compute_structure_index(data, label, n_bins=10, dims=None, **kwargs):
    ...
    # i) data input
    assert data.ndim == 2, (
        "Input 'data' must be a 2D numpy ndarray with shape"
    ...
    if isinstance(n_bins, int) or isinstance(n_bins, np.integer):
        assert n_bins > 1, "Input 'n_bins' must be an int or list of int larger than 1."
        n_bins = [n_bins for nb in range(label.shape[1])]
    elif isinstance(n_bins, list):
        assert np.all(
            [nb > 1 for nb in n_bins]
    ...
    if ("n_neighbors" in kwargs) and ("radius" in kwargs):
        raise ValueError(
            "Both n_neighbors and radius provided. Please only" + " specify one"
        )
    ...
    if "radius" in kwargs:
        ...
        if "n_neighbors" in kwargs:
            ...
            assert neighborhood_size > 2, (
                "Input 'n_neighbors' must be larger" + "than 2."
            )
        else:
    ...
    if "discrete_label" in kwargs:
        ...
        if isinstance(discrete_label, bool):
            ...
            assert np.all(
                [isinstance(idx, bool) for idx in discrete_label]
    ...
    if "num_shuffles" in kwargs:
        ...
        assert num_shuffles >= 0, (
            "Input 'num_shuffles must fall within the " + "interval [0, np.inf)"
        )
    else:
        num_shuffles = 100
    # xii) verbose input
    ...
    for dim in range(label.shape[1]):
        ...
        if discrete_label[dim]:
            ...
            warnings.warn(
                f"Along column {dim}, input 'label' has less unique "
    ...
    if verbose:
        bar = tqdm(total=num_shuffles, desc="Computing shuffling")
...
def draw_graph(
    overlap_mat, ax, node_cmap=plt.cm.tab10, edge_cmap=plt.cm.Greys, **kwargs
):
    ...
    if int(nx.__version__[0]) < 3:
        ...
        g = nx.from_numpy_array(
            overlap_mat, create_using=nx.DiGraph
