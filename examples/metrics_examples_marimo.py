# Marimo-style notebook: Metrics Examples
# This is a Python file that can be run as a script or used with marimo
...
from neural_analysis.plotting import add_trace_to_subplot
...
plot_specs = []
for i, (scenario_name, (p1, p2, desc)) in enumerate(scenarios.items()):
    # First distribution (blue)
    plot_specs.append(
        PlotSpec(
            data=p1[:, :2],  # First 2 dimensions
            plot_type="scatter",
    ...
    plot_specs.append(
        PlotSpec(
            data=p2[:, :2],
            plot_type="scatter",
            subplot_position=i,  # Same subplot as above
            label="Dist 2",
            color="red",
            alpha=0.4,
            marker_size=3,
...
for i, metric in enumerate(metrics):
    ...
    line_data = np.column_stack([shifts, sensitivity_results[metric]])
...
grid = PlotGrid(
    plot_specs=plot_specs,
    config=PlotConfig(title="Metric Sensitivity to Distribution Shift", figsize=(8, 5)),
    layout=GridLayoutConfig(rows=1, cols=1),
    backend="plotly",
)

fig = grid.plot()
fig.update_xaxes(title_text="Shift Magnitude (std devs)")
fig.update_yaxes(title_text="Distance/Similarity")
fig.show()
...
if "summary" in results:
    ...
    if distances:
        ...
        fig.update_yaxes(title_text="Mean Distance")
        fig.show()
...
for idx, method in enumerate(methods, start=1):
    _, mask = filter_outlier(
        data_with_outliers, method=method, contamination=contamination, return_mask=True
    )
...
conditions = {
    ...
    "Stimulus_A": np.random.randn(n_trials, n_neurons) * 0.5
    + np.array([1.0] * n_neurons),
    "Stimulus_B": np.random.randn(n_trials, n_neurons) * 0.8
...
from neural_analysis.plotting import plot_heatmap
...
for angle in angles:
    ...
    for _ in range(n_trials_per_angle):
        ...
        neural_activity = (
            projection_matrix @ manifold_activity + np.random.randn(n_neurons) * 0.1
        )
