...
def _(StorageConfig, bench_configs, output_dir, pd, run_benchmark, set_config):
    ...
    agg_df = benchmark_df.groupby(['label', 'description', 'use_cache', 'use_sql_index'], as_index=False).agg(write_seconds_mean=('write_seconds', 'mean'), write_seconds_std=('write_seconds', 'std'), load_seconds_mean=('load_seconds', 'mean'), load_seconds_std=('load_seconds', 'std'), file_mb_mean=('file_mb', 'mean'), rows_mean=('rows', 'mean')).sort_values('load_seconds_mean')
    ...
    recommended_cfg = StorageConfig(use_redis=_recommended_use_cache, use_sql=recommended_use_sql, redis_host='localhost', redis_port=6379, sql_path=output_dir / f'{recommended_label}.duckdb')
