...
def _(base, h5io, np):
    ...
    _attrs = {'description': 'random normal features', 'version': 1, 'metadata': {'source': 'synthetic', 'dims': list(data.shape)}}
...
def _(base, h5io, np, pd):
    ...
    _df = pd.DataFrame({'neuron_id': [f'n{i}' for i in range(5)], 'firing_rate': np.random.rand(5), 'condition': ['A', 'B', 'A', 'B', 'A']})
    ...
    pd.testing.assert_frame_equal(loaded_df.reset_index(drop=True), _df.reset_index(drop=True))
...
def _(base, display, h5io, load_hdf5, pd):
    ...
    pairs_df = pd.DataFrame({'item_i': ['A', 'A', 'B', 'C'], 'item_j': ['B', 'C', 'C', 'D'], 'score': [0.1, 0.8, 0.5, 0.9]})
    h5io(path_pairs, task='save', data=pairs_df, labels=None)
    wanted = [('A', 'C'), ('B', 'C')]
    (loaded_filtered, _), _attrs = load_hdf5(path_pairs, filter_pairs=wanted, return_attrs=True)
    print('Filtered rows:')
...
def _(base, np):
    ...
    from neural_analysis.utils.io import save_result_to_hdf5_dataset, load_results_from_hdf5_dataset, get_hdf5_dataset_names, get_hdf5_result_summary
    ...
    # Save multiple results under different comparison groups
            ...
            save_result_to_hdf5_dataset(save_path=hdf5_path, dataset_name=session, result_key=_result_key, scalar_data={'dataset_i': condition_pair[0], 'dataset_j': condition_pair[1], 'metric': 'wasserstein', 'value': float(value), 'n_samples_i': 100, 'n_samples_j': 100, 'timestamp': '2024-01-15'}, array_data={'pairs_indices': _pairs_indices, 'pairs_values': pairs_vals})  # Generate sample results
    print(f'✓ Saved hierarchical results to {hdf5_path.name}')
    print(f'  Structure: dataset_name / result_key / {{scalar_data, array_data}}')  # Scalar metric  # Generate pairwise comparison data (could be large arrays)  # Create structured array that HDF5 can handle  # Store as separate arrays: indices and values  # Save with scalar attributes and array datasets
    return (
...
def _(cache_file, compare_datasets, control_data, time, treatment_data):
    ...
    _start = time.time()
    result1 = compare_datasets(control_data, treatment_data, mode='between', metric='wasserstein', save_path=cache_file, dataset_names=('control', 'treatment'))
...
def _(
    ...
    _start = time.time()
    result2 = compare_datasets(control_data, treatment_data, mode='between', metric='wasserstein', save_path=cache_file, dataset_names=('control', 'treatment'), regenerate=False)
    ...
    print(f'   Results match: {result2_value == result1_value}')  # Default: use cached result
...
def _(
    ...
    _start = time.time()
    result3 = compare_datasets(control_data, treatment_modified, mode='between', metric='wasserstein', save_path=cache_file, dataset_names=('control', 'treatment'), regenerate=True)
    ...
    # Result is a dict with 'value' key for between mode
    print(f'   Changed: {abs(result3_value - result1_value) > 0.001}')  # Force recomputation
    return
...
def _(base, compare_datasets, np, time):
    ...
    datasets = {'control': np.random.randn(50, 8), 'treatment_A': np.random.randn(50, 8) + 0.3, 'treatment_B': np.random.randn(50, 8) + 0.7, 'treatment_C': np.random.randn(50, 8) + 1.0}
    ...
    _start = time.time()
    all_pairs_results = compare_datasets(datasets, mode='all-pairs', metric='wasserstein', save_path=cache_all_pairs, show_progress=False)
...
def _(
    ...
    _start = time.time()
    loaded_results = compare_datasets(datasets, mode='all-pairs', metric='wasserstein', save_path=cache_all_pairs, regenerate=False)
    elapsed_load = time.time() - _start
    print(f'   Time: {elapsed_load:.4f}s')
    print(f'   Speedup: {elapsed_compute / elapsed_load:.1f}x faster!')
    matches = all((loaded_results[k1][k2] == all_pairs_results[k1][k2] for k1 in all_pairs_results for k2 in all_pairs_results[k1]))
...
def _(cache_file, result2_value):
    ...
    direct_load = load_comparison(cache_file, metric='wasserstein', dataset_i='control', dataset_j='treatment')
