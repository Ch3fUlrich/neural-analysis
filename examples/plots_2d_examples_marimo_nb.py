...
def _(PlotConfig, np, plot_scatter_2d, plt, set_backend):
    ...
    # Create scatter plot
    _config = PlotConfig(title='Basic 2D Scatter Plot', xlabel='X Position', ylabel='Y Position')
...
def _(PlotConfig, np, plot_scatter_2d, plt):
    ...
    _config = PlotConfig(title='Neural Activity by Position', xlabel='X Position (mm)', ylabel='Y Position (mm)')
    # Create scatter plot with color mapping
    _fig = plot_scatter_2d(_x, _y, colors=firing_rates, cmap='viridis', colorbar=True, colorbar_label='Firing Rate (Hz)', config=_config)
    plt.show()
    return
...
def _(PlotConfig, np, plot_scatter_2d, plt):
    ...
    _config = PlotConfig(title='Variable Marker Sizes', xlabel='X Position', ylabel='Y Position')
    # Create scatter plot with variable sizes
    _fig = plot_scatter_2d(_x, _y, colors='coral', sizes=sizes, alpha=0.6, config=_config)
    plt.show()
    return
...
def _(PlotConfig, np, plot_trajectory_2d, plt):
    ...
    _config = PlotConfig(title='Neural State Trajectory Over Time', xlabel='PC1', ylabel='PC2')
    # Plot trajectory with time-based colors
    _fig = plot_trajectory_2d(_x_traj, _y_traj, color_by='time', cmap='plasma', linewidth=2, show_points=True, config=_config)
    plt.show()
    return
...
def _(PlotConfig, np, plot_trajectory_2d, plt):
    ...
    _config = PlotConfig(title='Movement Trajectory (Figure-8 Pattern)', xlabel='X Position (cm)', ylabel='Y Position (cm)')
    # Plot trajectory without time-based colors
    _fig = plot_trajectory_2d(_x_traj, _y_traj, color_by=None, linewidth=3, show_points=False, config=_config)
    plt.show()
    return
...
def _(PlotConfig, np, plot_grouped_scatter_2d, plt):
    ...
    grouped_data = {'Interneurons': (group1_x, group1_y), 'Pyramidal': (group2_x, group2_y), 'Glial': (group3_x, group3_y)}
    _config = PlotConfig(title='Cell Types in 2D Embedding Space', xlabel='Component 1', ylabel='Component 2')
...
def _(PlotConfig, np, plot_kde_2d, plt):
    ...
    _config = PlotConfig(title='Spatial Density of Spike Locations', xlabel='X Position (mm)', ylabel='Y Position (mm)')
    _fig = plot_kde_2d(x_kde, y_kde, cmap='RdYlBu_r', fill=True, show_points=False, config=_config)
...
def _(PlotConfig, plot_kde_2d, plt, x_kde, y_kde):
    ...
    _config = PlotConfig(title='KDE Contours with Data Points', xlabel='Feature 1', ylabel='Feature 2')
    _fig = plot_kde_2d(x_kde, y_kde, cmap='viridis', fill=False, show_points=True, n_levels=8, config=_config)
...
def _(PlotConfig, np, plot_scatter_2d, plt, set_backend):
    ...
    fig_mpl = plot_scatter_2d(_x, _y, colors=colors_data, colorbar=True, colorbar_label='Distance', config=config_mpl)
    plt.show()
    set_backend('plotly')
    config_plotly = PlotConfig(title='Plotly Backend (Interactive)', xlabel='X', ylabel='Y')
    # Plotly version (interactive)
    fig_plotly = plot_scatter_2d(_x, _y, colors=colors_data, colorbar=True, colorbar_label='Distance', config=config_plotly)
    return
