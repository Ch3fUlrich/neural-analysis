...
def _auto_select_test_method(data1: np.ndarray, data2: np.ndarray | None
    ...
    if test_type == 'paired' and data2 is not None and len(data1) != len(data2
        ...
        raise ValueError(
            'data1 and data2 must have the same length for paired tests')
    diagnostics = {'alpha_normality': alpha_normality,
        'min_sample_size_parametric': min_sample_size_parametric,
        'assumptions_note':
        'Parametric: normality (and equal variances for ttest_ind). Wilcoxon: symmetric differences. Mann-Whitney U: similar shapes.'
        }
    if test_type == 'paired':
        if data2 is not None:
            ...
            differences = data1
            diagnostics['note'
                ] = 'data1 treated as differences since data2 is None'
        n = len(differences)
        ...
        if n >= 3:
            try:
                ...
                diagnostics['normality_error'
                    ] = f'Shapiro-Wilk test failed: {str(e)}'
        else:
            diagnostics['normality_p'] = np.nan
        ...
        sufficient_size = (n1 >= min_sample_size_parametric and n2 >=
        ...
        if min(n1, n2) < 3:
            ...
            try:
                ...
                if var_p >= alpha_normality:
                    method = 'ttest_ind'
                    diagnostics['reason'
                        ] = 'parametric_assumptions_met_equal_variances'
                else:
                ...
                method = 'mannwhitneyu'
                diagnostics['reason'
                    ] = 'variance_test_failed_fallback_to_nonparametric'
        else:
            method = 'mannwhitneyu'
            diagnostics['reason'] = ('normality_violated' if not
                both_normal else 'small_sample_size')
        if max(n1, n2) > 50:
            diagnostics['warning'] = (
                'Shapiro-Wilk may be overly sensitive for large samples (>50). Consider visual checks.'
                )
...
def _auto_select_correction_method(num_comparisons: int, test_type: Literal
    ...
    """
    diagnostics = {'num_comparisons': num_comparisons, 'test_type':
        test_type, 'alpha': alpha}
    if test_type == 'paired':
        thresholds = {'bonferroni': 2, 'sidak': 2, 'holm': 5, 'holm-sidak':
            5, 'simes-hochberg': 5, 'hommel': 10, 'fdr_bh': 15, 'fdr_by':
            float('inf'), 'fdr_tsbh': float('inf'), 'fdr_tsbky': float('inf')}
    else:
        thresholds = {'bonferroni': 3, 'sidak': 3, 'holm': 8, 'holm-sidak':
            8, 'simes-hochberg': 8, 'hommel': 15, 'fdr_bh': 25, 'fdr_by':
            float('inf'), 'fdr_tsbh': float('inf'), 'fdr_tsbky': float('inf')}
    if num_comparisons <= thresholds['bonferroni']:
        ...
        diagnostics['alternatives'] = {'sidak':
            'similar to bonferroni but slightly less conservative', 'holm':
            'step-down procedure for small numbers', 'none':
            'no correction (not recommended for multiple comparisons)'}
    elif num_comparisons <= thresholds['holm']:
        method = 'holm'
        diagnostics['reason'] = (
            'small number of comparisons; holm provides good balance of power and control'
            )
        diagnostics['alternatives'] = {'holm-sidak':
            'combination of holm and sidak procedures', 'simes-hochberg':
            'less conservative than holm for independent tests',
            'bonferroni': 'more conservative option', 'fdr_bh':
            'FDR control instead of FWER'}
    elif num_comparisons <= thresholds['hommel']:
        method = 'hommel'
        diagnostics['reason'] = (
            'moderate number of comparisons; hommel improves on holm for general dependence'
            )
        diagnostics['alternatives'] = {'fdr_bh':
            'FDR control with good power', 'holm':
            'more conservative FWER control', 'fdr_by':
            'robust FDR for dependencies'}
    elif num_comparisons <= thresholds['fdr_bh']:
        method = 'fdr_bh'
        diagnostics['reason'
            ] = 'moderate-large number; FDR control with fdr_bh'
        diagnostics['alternatives'] = {'fdr_by':
            'more robust for positive dependencies', 'hommel':
            'FWER control with good power', 'fdr_tsbh':
            'two-stage FDR procedure', 'fdr_tsbky':
            'two-stage FDR with Hochberg step'}
    else:
        method = 'fdr_by'
        diagnostics['reason'
            ] = 'large number of comparisons; robust FDR control with fdr_by'
        diagnostics['alternatives'] = {'fdr_tsbh':
            'two-stage FDR procedure for large numbers', 'fdr_tsbky':
            'two-stage FDR with Hochberg step', 'fdr_bh':
            'standard FDR (less robust to dependencies)', 'hommel':
            'FWER control (more conservative)'}
    return method, diagnostics


def check_normality(data: np.ndarray, min_size: int=3, context: str=''
    ) ->float:
    """
...
def apply_multiple_correction(p_values: list[float], method: str, alpha:
    ...
    if valid_mask.any():
        corrected[valid_mask] = multipletests(p_values_array[valid_mask],
...
def _compute_paired_pvalue(values1: np.ndarray, values2: np.ndarray, method:
    ...
    norm_p = check_normality(differences, context=
        f"for difference '{labels[0]}' vs '{labels[1]}' in '{group_name}' for '{col}'"
         if labels and group_name and col else 'for paired difference')
    current_method = method
    if method == 'auto':
        current_method, auto_diagnostics = _auto_select_test_method(data1=
            values1, data2=values2, test_type='paired')
    ...
    if current_method == 'tukey':
        ...
        tukey_labels = [labels[0], labels[1]] if labels else ['group1',
            'group2']
        ...
        try:
            if current_method == 'wilcoxon':
                ...
                res = permutation_test((values1, values2), mean_diff,
                    vectorized=False, permutation_type='samples',
                    alternative='two-sided', n_resamples=n_permutations)
                p_value = res.pvalue
            elif current_method in ['monte_carlo_test',
                ...
                if current_method == 'monte_carlo_test_normal':
                    ...
                    rvs1 = rvs2 = lambda size: np.random.choice(combined,
                        size, replace=True)
                res = monte_carlo_test((values1, values2), statistic=
                    mean_diff, vectorized=False, alternative='two-sided',
                    rvs=(rvs1, rvs2))
                p_value = res.pvalue
            else:
...
def _compute_unpaired_pvalue(values1: np.ndarray, values2: np.ndarray,
    ...
    if normality:
        norm_p1 = check_normality(values1, context=
            f"for group '{labels[0] if labels else 'group1'}' in '{group_name}' for '{col}'"
             if labels and group_name and col else 'for first group')
        norm_p2 = check_normality(values2, context=
            f"for group '{labels[1] if labels else 'group2'}' in '{group_name}' for '{col}'"
             if labels and group_name and col else 'for second group')
    current_method = method
    if method == 'auto':
        current_method, auto_diagnostics = _auto_select_test_method(data1=
            values1, data2=values2, test_type='unpaired')
        if labels and group_name and col:
            logger.info(
...
def compute_tukey_pvalues(data_list: list[np.ndarray], labels: list[str]
    ...
    all_groups = np.concatenate([([labels[i]] * len(data_list[i])) for i in
        range(len(labels))])
...
def statistical_comparison(df: pd.DataFrame, pair_name_col: str, compare_by:
    ...
    if 'paired' in test_types:
        ...
        paired_results = _run_statistical_tests(df=df, pair_name_col=
            pair_name_col, compare_by=compare_by, value_col=value_col,
            group_by=group_by, groups=groups, labels=labels, method=
            paired_method, correction_method=correction_method,
            n_permutations=n_permutations, normality=normality, paired=True)
    ...
    if 'unpaired' in test_types:
        ...
        if unpaired_possible:
            ...
            unpaired_method = (method if method in unpaired_methods else
                'mannwhitneyu')
            if method not in unpaired_methods and method != 'auto':
                logger.warning(
                    f"Method '{method}' not valid for unpaired tests. Using 'mannwhitneyu'."
                    )
            unpaired_results = _run_statistical_tests(df=df, pair_name_col=
                pair_name_col, compare_by=compare_by, value_col=value_col,
                group_by=group_by, groups=groups, labels=labels, method=
                unpaired_method, correction_method=correction_method,
                n_permutations=n_permutations, normality=normality, paired=
                False)
...
def _initialize_heatmaps(value_col: list[str], groups: list[str], labels:
    ...
    """
    if paired:
        test_heatmaps = {col: {group: pd.DataFrame(np.nan, index=labels,
            columns=labels) for group in groups} for col in value_col}
        normality_heatmaps = None
        if normality:
            normality_heatmaps = {col: {group: pd.DataFrame(np.nan, index=
                labels, columns=labels) for group in groups} for col in
                value_col}
    else:
        test_heatmaps = {col: pd.DataFrame(np.nan, index=labels, columns=[
            col]) for col in value_col}
        normality_heatmaps = None
        if normality:
            normality_heatmaps = {col: pd.DataFrame(np.nan, index=labels,
                columns=groups) for col in value_col}
...
def _run_statistical_tests(df: pd.DataFrame, pair_name_col: str, compare_by:
    ...
    num_comparisons = num_value_cols * groups_count * (labels_count * (
        labels_count - 1) // 2) if paired else num_value_cols * labels_count
    selected_correction_method = correction_method
    if correction_method == 'auto':
        selected_correction_method, auto_diagnostics = (
            _auto_select_correction_method(num_comparisons, test_type=
    ...
    test_heatmaps, normality_heatmaps = _initialize_heatmaps(value_col,
        groups, labels, normality, paired)
        ...
        if paired:
            ...
            for label1, label2 in combs:
                ...
                if not common_names or len(common_names) < 2:
                    ...
                    continue
                for col in value_col:
                    values1 = group_label1_df[group_label1_df[pair_name_col
                        ].isin(common_names)][col].values
                    values2 = group_label2_df[group_label2_df[pair_name_col
                        ].isin(common_names)][col].values
                    p_value, norm_p = _compute_paired_pvalue(values1,
                        values2, method, n_permutations, (label1, label2),
                        group_name, col)
                    if normality:
            ...
            for col in value_col:
                for label in labels:
                    ...
                    groups_data = [label_df[label_df[group_by] == g][col].
                    ...
                    p_value, norm_p1, norm_p2 = _compute_unpaired_pvalue(
                        groups_data[0], groups_data[1], method, groups,
                        label, col, normality)
                    if normality:
                    ...
                    all_p_values.append(p_value)
                    all_comparisons.append((col, label))
        corrected_p = apply_multiple_correction(all_p_values,
            selected_correction_method)
        if paired:
            for idx, (col, label1, label2) in enumerate(all_comparisons):
                test_heatmaps[col][group_name].loc[label1, label2
                    ] = corrected_p[idx]
                test_heatmaps[col][group_name].loc[label2, label1
                    ] = corrected_p[idx]
        else:
...
def mannwhitneyu_cross_df(df: pd.DataFrame, group_by: str, compare_by: str,
    ...
    for label in labels:
        ...
        if len(value_col) > 1:
            ...
            if valid_mask.any():
                corrected_p = multipletests(p_values[valid_mask], method=
                    'holm', alpha=0.05)[1]
                pvalues_df.loc[label, value_col] = np.where(valid_mask,
                    corrected_p, np.nan)
            else:
...
--- P-values for {value_name} ---")
    ...
    if normality:
        normality_heatmaps = {col: {group_name: pd.DataFrame(np.nan, index=
            labels, columns=labels) for group_name in groups} for col in
            value_col}
    num_animals_dict = {group_name: pd.DataFrame(0, index=labels, columns=
        labels, dtype=int) for group_name in groups}
    for group_name in groups:
        group_df = df if group_by is None else df[df[group_by] == group_name]
        group_df = group_df.sort_values(by=pair_name_col)
        for col in value_col:
        ...
        if method == 'tukey':
            ...
            if normality:
                for col in value_col:
                    for label in labels:
                        values = group_df[group_df[compare_by] == label][col
                            ].dropna().values
                        if len(values) >= 3:
                            _, norm_p = shapiro(values)
                            normality_heatmaps[col][group_name].loc[label,
                                label] = norm_p
                            if norm_p < 0.05:
                                logger.warning(
            ...
            for col in value_col:
                ...
                groups_data = [group_df[group_df[compare_by] == label][col]
                    .dropna().values for label in labels_sorted]
                if len(labels_sorted) < 2 or any(len(g) == 0 for g in
                    groups_data):
                    tukey_results[col] = None
                    continue
                all_values = np.concatenate(groups_data)
                all_groups = np.concatenate([([labels_sorted[i]] * len(
                    groups_data[i])) for i in range(len(groups_data))])
                res = pairwise_tukeyhsd(all_values, all_groups, alpha=0.05)
                p_matrix = pd.DataFrame(np.nan, index=labels_sorted,
            ...
            for label1, label2 in combs:
                ...
                common_names = set(group_label1_df[pair_name_col]) & set(
                    group_label2_df[pair_name_col])
                num_animals_dict[group_name].loc[label1, label2] = len(
                    common_names)
                num_animals_dict[group_name].loc[label2, label1] = len(
                    common_names)
                if not common_names or len(common_names) < 2:
                    for col in value_col:
                        all_p_values.append(np.nan)
                ...
                for common_name in common_names:
                    if group_label1_df[group_label1_df[pair_name_col] ==
                        common_name].shape[0] != 1:
                        do_critical(ValueError,
                            f"Column '{pair_name_col}' has more than one unique value in group '{label1}' for common name '{common_name}'."
                            )
                    if group_label2_df[group_label2_df[pair_name_col] ==
                        common_name].shape[0] != 1:
                        do_critical(ValueError,
                ...
                for col in value_col:
                    ...
                    if normality:
                        ...
                        if len(differences) >= 3:
                            _, norm_p = shapiro(differences)
                            normality_heatmaps[col][group_name].loc[label1,
                                label2] = norm_p
                            normality_heatmaps[col][group_name].loc[label2,
                                label1] = norm_p
                            if norm_p < 0.05:
                                logger.warning(
                                    f"Warning: Normality test p-value {norm_p:.4f} < 0.05 for difference between '{label}' and its baseline in group '{group_name}' for column '{col}'. Consider using a non-parametric test."
                    ...
                    try:
                        if method == 'wilcoxon':
                            ...
                            res = permutation_test((values1, values2),
                                mean_diff, vectorized=False,
                                permutation_type='samples', alternative=
                                'two-sided', n_resamples=n_permutations)
                            p_value = res.pvalue
                        elif method == 'monte_carlo_test' or method == 'monte_carlo_test_normal':
                            combined = np.concatenate([values1, values2])
                            mu, sigma = np.mean(combined), np.std(combined)
                            if 'normal' in method:
                                rvs = lambda size: np.random.normal(mu,
                                    sigma, size=size
                                    ), lambda size: np.random.normal(mu,
                                    sigma, size=size)
                            else:
                                rvs = lambda size: np.random.choice(combined,
                                    size=size, replace=True
                                    ), lambda size: np.random.choice(combined,
                                    size=size, replace=True)
                            res = monte_carlo_test(data=(values1, values2),
                                rvs=rvs, statistic=mean_diff, vectorized=
        ...
        if correction_method != 'none' and all_p_values:
            ...
            if valid_mask.any():
                corrected_p[valid_mask] = multipletests(all_p_values[
                    valid_mask], method=correction_method, alpha=0.05)[1]
            for idx, (col, label1, label2) in enumerate(all_comparisons):
                test_heatmaps[col][group_name].loc[label1, label2
                    ] = corrected_p[idx]
                test_heatmaps[col][group_name].loc[label2, label1
