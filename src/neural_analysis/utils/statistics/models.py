...
FunctionType = Callable[[np.ndarray, float, float | None, float | None], np.ndarray]  # type: ignore
from typing import Literal  # noqa: E402

from scipy.optimize import curve_fit  # noqa: E402

from neural_analysis.utils.common.collections import (  # noqa: E402
    make_list_ifnot,
)
...
class FunctionModel:
    ...
    @staticmethod
    def shift_x(x: np.ndarray, x_shift: float) -> np.ndarray:  # type: ignore
        """Apply horizontal shift to x values."""
        return x - x_shift

    @staticmethod
    def shift_y(y: np.ndarray, y_shift: float) -> np.ndarray:  # type: ignore
        """Apply vertical shift to y values."""
        return y + y_shift

    @classmethod
    def apply_inverse(cls, y: np.ndarray, inverse: bool) -> np.ndarray:  # type: ignore
...
def get_auc(
    ...
    if method == "fast":
        ...
        auc = fit_result.get("auc", np.nan)  # type: ignore
        if auc is np.nan:
            logger.error(
            ...
            auc = np.trapezoid(y, x)
        if return_fit:
            return auc, fit_result  # type: ignore
    return auc
...
def get_best_fit(
    ...
    inverse: bool = False,
    maxfev: int = 10000,
) -> dict[str, float | list[float] | np.ndarray | str]:  # type: ignore
    """
    ...
    fits: dict[str, dict] = {}  # type: ignore
    implemented_functions = ["linear", "exponential", "gaussian", "hyperbolic", "power"]
    function_map: dict[str, FunctionType] = {
        "linear": FunctionModel.linear,  # type: ignore
        "exponential": FunctionModel.exponential,  # type: ignore
        "gaussian": FunctionModel.gaussian,  # type: ignore
        "hyperbolic": FunctionModel.hyperbolic,  # type: ignore
        "power": FunctionModel.power,  # type: ignore
    }
    if functions is None or isinstance(functions, str) and functions == "auto":
        functions = implemented_functions  # type: ignore
    else:
    ...
    initial_guesses = {"positive": [0.1, 2.0, 1.0], "negative": [-0.1, -2.0, 1.0]}
        ...
        try:
            try:
                if func_name == "power":
                    ...
                    popt, _ = curve_fit(
                        lambda x, k, n, a, f=func, c=config: f(x, k, n, a, c),
                        x,
                        y,
                        p0=p0,
                        bounds=bounds,
                    ...
                    y_pred = func(x, *popt, config)  # type: ignore
                    y_dense = func(x_dense, *popt, config)  # type: ignore
                    param = {"k": popt[0], "n": popt[1], "a": popt[2]}
                else:
                    p0 = [0.1, 1.0]
                    bounds = [0, -np.inf], [np.inf, np.inf]
                    popt, _ = curve_fit(
                        lambda x, k, a, f=func, c=config: f(x, k, a, c),
                        x,
                        y,
                        p0=p0,
                        bounds=bounds,
