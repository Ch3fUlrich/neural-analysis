...
def plot_bar(
    ...
    backend: Literal["matplotlib", "plotly"] | None = None, **kwargs) -> Any:
    """
...
def plot_grouped_distributions(
    ...
    **kwargs
    ...
    if colors is None:
        colors = [
            DEFAULT_COLORS[i % len(DEFAULT_COLORS)] for i in range(len(conditions))
        ]
...
def plot_comparison_distributions(
    ...
    **kwargs
    ...
    if is_nested:
        ...
        if colors is None:
            colors = [
                DEFAULT_COLORS[i % len(DEFAULT_COLORS)] for i in range(len(groups))
            ]
