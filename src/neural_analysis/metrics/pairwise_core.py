"""Core pairwise metrics: distances, similarities, correlations, autocorrelation.

This module contains the non-numba core computation functions for pairwise
distance metrics, similarity matrices, correlation functions, and spatial
...
from .pairwise_numba import (  # noqa: E402
    NUMBA_AVAILABLE,
    _correlation_matrix_parallel,
)

if NUMBA_AVAILABLE:
    from .pairwise_numba import (
...
def _get_distance_function(metric: DistanceMetric) -> Any:
    match metric:
        ...
        case _:
            raise ValueError(
                f"Unknown distance metric. Expected: 'euclidean', 'manhattan', "
                f"'mahalanobis', or 'cosine'. Got: {metric!r}"
            )
...
def pairwise_distance(
    ...
    **metric_kwargs
    ...
    match metric:
        ...
        case _:
            raise ValueError(
                f"Unknown pairwise distance metric. Expected: 'euclidean', "
                f"'manhattan', 'cosine', or 'mahalanobis'. Got: {metric!r}"
            )
...
def compute_between_distances(
    ...
    **metric_kwargs
    ...
    if metric_normalized in POINT_TO_POINT_METRICS:
        ...
        raise ValueError(
            f"Unexpected metric type. Expected one of: "
            f"{', '.join(sorted(ALL_METRICS))}. Got: {metric_normalized!r}"
        )
...
# ---------------------------------------------------------------------------
# Correlation and similarity functions
# ---------------------------------------------------------------------------
...
def correlation(
    ...
    if mode == "matrix":
        ...
        raise ValueError(
            f"Unknown correlation mode. Expected: 'matrix' or 'pairwise'. Got: {mode!r}"
        )
...
def similarity_matrix(
    ...
    match method:
        ...
        case _:
            raise ValueError(
                f"Unknown similarity method. Expected: 'pearson', 'spearman', "
                f"'kendall', 'cosine', or 'angular'. Got: {method!r}"
            )
...
# ---------------------------------------------------------------------------
# Parallel similarity helpers (non-numba, numpy-based)
# ---------------------------------------------------------------------------
