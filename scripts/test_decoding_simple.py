...
try:
    ...
    print(f"  Available functions: {[f for f in dir(decoding) if not f.startswith('_')]}")
...
try:
    activity, meta = generate_place_cells(n_cells=30, n_samples=500, n_dims=2, seed=42)
    ...
    if mean_error < 0.3:
        ...
        print(f"✗ Population vector decoder test FAILED (error too high: {mean_error:.4f})")
...
print("\n=== Testing knn_decoder ===")
try:
    activity, meta = generate_place_cells(n_cells=40, n_samples=800, n_dims=2, seed=123)
    print(f"✓ Generated place cells: {activity.shape}")
    ...
    decoded = knn_decoder(
        train_act, train_pos, test_act, k=5, weights='distance'
    )
...
try:
    activity, meta = generate_place_cells(n_cells=45, n_samples=1000, n_dims=2, seed=456)
    print(f"✓ Generated place cells: {activity.shape}")
...
try:
    ...
    activity, meta = generate_place_cells(n_cells=80, n_samples=1200, n_dims=2, seed=789)
    print(f"✓ Generated place cells: {activity.shape}")
