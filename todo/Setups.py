...
class Output:
    ...
    def define_file_path(self, file_name=None, full_root_dir: Path = None):
        ...
        if fit_file_name not in transposed_output_paths.keys():
            for existing_file_name, path in transposed_output_paths.items():
                file_name_match = regex_search(
                    existing_file_name, include_regex=fit_file_name
                )
                if file_name_match:
    ...
    def load_data(
        ...
        if fpath.exists():
            ...
            global_logger.warning(f"File {file_name} not found in {fpath}")
        return data
    ...
    def save_data(self, data, save_dir=None, overwrite=False):
        for data_name, data_type in data.items():
            ...
            if fpath.exists() and not overwrite:
                global_logger.info(f"File {fpath} already exists. Skipping.")
            else:
...
class Treadmill_Setup(BehavioralSetup):
    ...
    def __init__(self, key, root_dir=None, metadata={}):
        ...
        self.data_path = (
            self.define_data_path()
        )  # file_name=self.output_data_naming_scheme
    ...
    def define_data_path(self, fname: str = None):
        ...
        if not self.data_path:
            ...
            if not data_path.exists():
                ...
                animal_id, date, task_name = self.identifier.values()
                regex_search = (
                    f"{animal_id}_{date}[{allowed_symbols}]*{task_name}[{allowed_symbols}]*"
                    + str(data_path.suffix)
                    + "$"
                )
                data_paths = search_filedir(
                    path=data_path.parent,
                    include_regex=regex_search,
                )
                if len(data_paths) == 0:
                    global_logger.critical(
                    ...
                    global_logger.error(f"File {data_path} not found.")
                    global_logger.warning(
                        f"Choosing first matching multi task dataset found in {data_path.parent}. Needed for imaging frame rate determination. Alternative could be to provide the fps rate in the neural metadata section in the yaml file."
                    )
...
class Cam(Behavior_Preprocessing):
    ...
    def pixel_to_meter(
        ...
        pixel_per_meter: float = None,
        env_dim: List[float] = None,
    ) -> np.ndarray:
        if pixel_per_meter is None:
            if env_dim is None:
                global_logger.critical(
                    f"No pixel_per_meter or environment dimensions provided, which is needed to convert pixel data to meters."
                )
                raise ValueError(
                    f"No pixel_per_meter or environment dimensions provided, which is needed to convert pixel data to meters."
                )
            global_logger.debug(
                f"No pixel_per_meter provided. Using max values and given environment dimensions {env_dim}."
            )

            filtered_pixel_data = Environment.remove_percentile(pixel_data)
            # remove the minimum value of the filtered pixel data to get the real 0,0 position
            filtered_pixel_data -= np.min(filtered_pixel_data, axis=0)
            max_values = np.max(filtered_pixel_data, axis=0)
            pixel_per_meter = max_values / np.array(env_dim)
        return filtered_pixel_data / pixel_per_meter
    ...
    def preprocess_data(self, cam_data, save=True, overwrite=False):
        ...
        """
        if keys_missing(self.metadata, ["environment_dimensions"]):
            env_dim = None
            if keys_missing(self.metadata, ["pixel_per_meter"]):
                raise ValueError(
                    f"No pixel_per_meter or environment dimensions provided, which is needed to convert pixel data to meters."
                )
            else:
                pixel_per_meter = self.metadata["pixel_per_meter"]
        elif keys_missing(self.metadata, ["pixel_per_meter"]):
            ...
            if keys_missing(self.metadata, ["environment_dimensions"]):
                ...
                env_dim = self.metadata["environment_dimensions"]
        ...
        position = Cam.pixel_to_meter(
            position_in_pixel, pixel_per_meter=pixel_per_meter, env_dim=env_dim
        )
...
class Environment(Behavior_Processing):
    ...
    @staticmethod
    def get_cumdist_from_position(
        position: np.ndarray, imaging_fps: float = None, smooth: bool = True
    ):
        """
        ...
        global_logger.debug(
            f"Max position: {np.max(position, axis=0)}. Setting big negative distance jumps to 0. Position reset is expected."
        )
    ...
    def get_velocity_from_cumdist(
        ...
        # append 0 to the beginning of the cumulative distance to match the shape of the velocity
        # ensure insertion has same dimensions as cumulative_distance
        zero_to_first = np.zeros((1, cumulative_distance.shape[1]))
        cumulative_distance = np.vstack(
            (zero_to_first, cumulative_distance)
        )
        ...
        velocity_smoothed = may_butter_lowpass_filter(
            velocity, smooth, cutoff=None, fps=imaging_fps, order=2
        )
        
        return velocity_smoothed

    @staticmethod
    def get_moving_from_velocity(
        velocities: np.ndarray,
        thr: float = 0.02,
        reset_time: Union[int, float] = None,
        fps: Union[int, float] = None,
    ) -> np.ndarray:
        """
        Get moving frames from velocity. The moving frames are defined as the frames where the velocity is above a certain threshold.

        If the reset_time is set, the moving frames are reset after a certain amount of time in seconds. Representing the time it takes for the brain to reset the statespace.
        ...
        ----------
            ...
            thr: float
                The threshold for the velocity to be considered as moving in meter per second. Default is 0.02 m/s.
            reset_time: int, optional (default=None)
                The time in seconds to reset the moving frames after a certain amount of time in seconds.
            fps: float, optional (default=None)
                The frames per second of the imaging data. Needed to reset the moving frames after a certain amount of time in seconds.

        Returns
        -------
            moving_frames: np.ndarray
                Array of moving frames. True if the frame is moving, False if not.
        """
        velocities_abs = np.linalg.norm(np.abs(velocities), axis=1)
        moving_frames = velocities_abs > thr
        if reset_time is not None:
            if fps is None:
                global_logger.error(
                    f"fps is None. Cannot reset moving frames after time delay. Set fps to the imaging fps. Returnning moving frames without reset time lag."
                )

            # fill array with value if lag is biggern than gap
            processing_movement_frames = fill_continuous_array(
                moving_frames, fps=fps, time_gap=reset_time
            )
        ...
        moving_frames = processing_movement_frames
    ...
    def get_acceleration_from_velocity(
        ...
        acceleration_smoothed = may_butter_lowpass_filter(
            acceleration, smooth, cutoff=None, fps=imaging_fps, order=2
        )
    ...
    @staticmethod
    def to_euclidean(data: np.ndarray) -> np.ndarray:
        """
        Convert multi-dimensional vectors to euclidean (magnitudes).

        This function was made for handling space.
        Handles 1D (treated as single vector), 2D (treated as multiple vectors),
        and 3D (treated as multiple vectors per timestep) inputs.

        Parameters:
            data: np.ndarray
                Input array of shape (x,) for 1D, (x,y) for 2D, or (x,y,z) for 3D

        Returns:
            np.ndarray
                Euclidean norms with shape (x, 1)

        Raises:
            ValueError: If input has more than 3 dimensions
        """
        if data.ndim > 3:
            raise ValueError(f"Data must be 1D, 2D or 3D. Got {data.ndim}D input.")

        # For 3D (t,n,m), compute norm over last axis and squeeze middle dimension
        norms = np.linalg.norm(abs(data), axis=-1)
        return norms.reshape(-1, 1)

    @staticmethod
    def remove_percentile(values: np.ndarray, percentile: int = 1):
        """
        Remove the percentile of the data from the values while maintaining the original shape.
        Values outside the percentile range are removed and replaced with the closest valid values.
        """
        ...
        lower_bound = np.percentile(values, percentile, axis=0)
        upper_bound = np.percentile(values, 100 - percentile, axis=0)

        # Create a copy of values
        filtered_values = values.copy()

        # For each column, replace outliers with the boundary values
        for i in range(values.shape[1]):
            column = filtered_values[:, i]
            # Replace values below lower bound with the lower bound
            column[column < lower_bound[i]] = lower_bound[i]
            # Replace values above upper bound with the upper bound
            column[column > upper_bound[i]] = upper_bound[i]

        # Restore the original shape
        if len(original_shape) == 1:
            filtered_values = filtered_values.reshape(original_shape)
    ...
    def define_border_by_pos(positions: np.ndarray, percentile: int = 1, map=False):
        ...
        if map:
            ...
            for dim in range(positions.shape[1]):
                filtered_positions = Environment.remove_percentile(
                    positions[:, dim], percentile=percentile
                )
                min_pos = np.min(filtered_positions)
                max_pos = np.max(filtered_positions)
    ...
    def data(self, type: str = "processed") -> np.ndarray:
        ...
        Args:
            type (str, optional): Loads data stored in the npz file which is created by the CaBinCorr processing. Defaults to "processed".
                Options:
                    - "unprocessed": corresponds to "F_fileterd" data corresponding to detrendet fluorescence.
                    - "processed": corresponds to "F_upphase" data corresponding to upphase of fluorescence signal.
                    ...
                    - "F_detrended": detrendet fluorescence.
        Returns:
