...
This module provides tools for creating and managing YAML metadata files from 
Excel spreadsheets and folder structures for the SERBRA pipeline.

The module automates the conversion of experimental metadata stored in Excel 
spreadsheets into structured YAML files that are required by the SERBRA data 
loading pipeline. It handles:
- Reading animal, session, and task metadata from Excel files
- Creating proper folder hierarchies following SERBRA conventions
- Generating YAML files for each animal and session
- Organizing raw data files (.mesc) into appropriate folders

Key Functions:
    get_animal_dict_from_spreadsheet: Extract metadata from Excel spreadsheets
    get_animals_from_yaml: Load existing YAML files from directories
    combine_spreadsheet_and_old_animal_summary_yaml: Merge spreadsheet and YAML data
    create_folders_for_animals: Create organized folder structure with YAML files
    move_mesc_to_session_folder: Organize .mesc files into session folders
    main: Main pipeline for Excel to YAML conversion

Workflow:
    1. Read metadata from Excel spreadsheet using get_animal_dict_from_spreadsheet()
    2. Load any existing YAML files using get_animals_from_yaml()
    3. Merge the data using combine_spreadsheet_and_old_animal_summary_yaml()
    4. Create folder structure and save YAML files using create_folders_for_animals()
    5. Organize data files using move_mesc_to_session_folder()

Dependencies:
    - pyyaml: YAML file handling
    - openpyxl: Excel file reading
    - h5py: Reading .mesc files
    - datetime: Date handling
    - Helper: Custom helper functions for file operations

Example:
    >>> from yaml_creator import get_animal_dict_from_spreadsheet, create_folders_for_animals
    >>> 
    >>> # Extract metadata from Excel
    >>> animals = get_animal_dict_from_spreadsheet(
    ...     fname='experiment_metadata.xlsx',
    ...     sheet_title='rigid-plastic'
    ... )
    >>> 
    >>> # Create folder structure with YAML files
    >>> create_folders_for_animals(animals, directory='/path/to/data')

Notes:
    - File naming must follow SERBRA conventions (see main README.md)
    - Animal IDs should be in format: DON-XXXXXX
...
def row_to_list(sheet, row):
    ...
    stopping at the first None value encountered.
    
    Parameters:
    ----------
        sheet: openpyxl worksheet object
            The Excel worksheet to read from
        
        row: int
            Row number to extract (1-indexed)
    
    Returns:
    -------
        result: list
            List of non-None cell values from the row
    """
    result = []
...
def num_to_date(date_string):
    ...
    Parameters:
    ----------
        date_string: str or int
            Date in YYYYMMDD format (e.g., '20210228' or 20210228)
...
def define_metadata_columns(sheet):
    ...
    metadata field names and their column positions.
...
def create_neural_dict(sheet, metadata_columns, row):
    ...
    laser_power = sheet.cell(
        row=row, column=metadata_columns["laser / LED power"]
    ).value
    ...
    functional_channel = sheet.cell(
        row=row, column=metadata_columns["fct. channel"]
    ).value
...
def create_task_dict(sheet, metadata_columns, row, stimulus_definition=None):
    ...
    duration = (
        None if duration == "n/a" or duration == "" or duration == "?" else duration
    )
...
def create_animal_dict(sheet, metadata_columns, row, stimulus_definition=None):
    ...
    if not animal_id:
        ...
        animal_id = (
            "DON-00" + animal_id[3:] if len(animal_id) == 7 else "DON-0" + animal_id[3:]
        )
...
def get_animal_dict_from_spreadsheet(
    ...
    for row in range(2, sheet.max_row):
        ...
        session_date = row_animal_metadata["sessions"][
            list(row_animal_metadata["sessions"].keys())[0]
        ]["date"]
            ...
            if animal_id in animals:
                ...
                if session_date in animal["sessions"]:
                    ...
                    session["tasks_metadata"].update(
                        row_animal_metadata["sessions"][session_date]["tasks_metadata"]
                    )
                else:
...
def create_dict(**kwargs):
    ...
    ----------
        **kwargs: keyword arguments
            Key-value pairs to include in dictionary
...
def remove_none_from_dict(dictionary, recursive=False):
    ...
    ----------
        ...
        recursive: bool (default: False)
            If True, also removes None values from nested dictionaries
